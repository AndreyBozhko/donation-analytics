...
public class MapOfMaps<T> {
    ...
    private final Map<String, Map<String, Map<String, T>>> map;
    ...
    public boolean containsKey(String key1, String key2, String key3) {
        boolean flag = map.containsKey(key1);

        if (flag) {
            flag = map.get(key1).containsKey(key2);
        }
        if (flag) {
            flag = map.get(key1).get(key2).containsKey(key3);
        }
